...
function mdlwp_scripts() {
	...
	wp_enqueue_style( 'mdlwp-mdl-css', '//static.cryse.org/blog/css/material.'.$primary.'-'.$secondary.'.min.css' );

	wp_enqueue_style( 'mdlwp-mdl-icons', '//fonts.lug.ustc.edu.cn/icon?family=Material+Icons' );

	wp_enqueue_style( 'mdlwp-mdl-roboto', '//fonts.lug.ustc.edu.cn/css?family=Roboto:300,400,500,700' );

	wp_enqueue_style( 'mdlwp-style', get_template_directory_uri() . '/style.min.css' );

	wp_enqueue_script( 'mdlwp-mdl-js', '//cdn.bootcss.com/material-design-lite/1.1.3/material.min.js', array(), '1.1.1', true );
